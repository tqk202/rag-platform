...
> 一个面向真实企业场景的检索增强生成（RAG）问答系统，突出**工程化落地能力**：多租户权限、文档生命周期、异步文档处理、全链路可观测、评测体系。

## 核心设计

围绕企业 RAG 落地的七个核心问题，本项目的设计逐条对应：
...
- **健康检查依赖链**：etcd（`etcdctl`）+ MinIO（curl）健康 → Milvus（curl `/healthz`）健康 → backend 才启动。**注意**：etcd 默认只监听 localhost，Milvus 在容器网络里连不上，必须 `command: etcd -listen-client-urls=http://0.0.0.0:2379 ...` 显式监听。
- **镜像加速**：Docker Hub 直连易超时，`~/.docker/daemon.json` 配置 `registry-mirrors`；构建阶段 Dockerfile 里也配了 pip 清华源 + npm 阿里镜像（否则 pip 直连 PyPI 卡 20 分钟+）。

**全链路 E2E 验收**：8 容器全健康 → 生产 seed 灌入 8 份企业级文档 / 76 切片（真实 bge-m3，1024 维向量入 PostgreSQL + Milvus）→ 注册/登录 → SSE 问答（真实 DeepSeek + 引文）→ 会话 → 改密（旧密码即失效）→ 越权防护（member 删文档 403 / admin 200）→ 删除联动清双存储。企业级加固后重跑：3 个表格/补偿类问题真实 LLM 全部带引文答对。
...
- **黄金评测集**：`backend/eval_data/golden_set.json`，**52 道可答题 + 3 道拒答题**「问题 + 标准答案出处」，基于 8 份企业脏文档手写。按题型覆盖单跳/表格取值/数值计算/条件逻辑/跨文档引用五类。
- **四个指标**：`context_precision` / `context_recall`（检索质量，用黄金答案出处算）+ `faithfulness` / `answer_relevancy`（生成质量，LLM 当裁判）。手写 RAGAS 风格实现，不依赖第三方评测黑盒。
- **拒答准确率（v2 新增）**：RAGAS 四指标只度量"可答题"的召回与生成质量，衡量不了"正确地不知道"——所以无关题单独算 `reject_accuracy`，不掺进四指标均值。CI 护栏同时守两条线：可答质量不得回退、拒答率不得下降。
...
**实测（全真实模式，18 题，早期历史数据）**——嵌入 bge-m3 + 重排 bge-reranker-v2-m3（SiliconFlow）+ 裁判 DeepSeek：
...
要点：mock 哈希嵌入下纯向量只有 0.21（无语义），换真实 bge-m3 后 **0.21 → 0.94**——语义嵌入的价值被直接量化；真实重排把无重排漏掉的 2 道题（0.5）修正到满分。

**当前评测（8 份企业脏文档，52+3 题，离线 mock 可跑）**：默认配置（混合检索 + 词法重排 + chunk500）四指标 **全 1.0**、可答 52 题全对、拒答 3 题全对（reject_accuracy=1.0）。对比旧 18 题干净语料：语料从 5 份/18 题升到 8 份/52 题、且文档带真实脏数据，满分含金量更高——在**脏文档**上还能检索到标准答案，才是管线真本事。
...
- **7 个边界测试**（`tests/test_edge_cases.py`）：坏扩展名 / 空文件 / 超大小 /
  空问题 / LLM 挂了 502 / SSE 错误事件 / 正常路径回归。全量 **113 个测试通过**（无 Redis 环境时真实 Redis 用例自动跳过）。
...
- 重试耗尽抛错后自动落进既有 502 兜底（`UPSTREAM_ERROR` / `UPSTREAM_UNAVAILABLE`），与错误分层机制无缝衔接，零新增异常路径。
...
在线问答路径与离线入库路径都需要重试——上传时嵌入接口抖动，任务不能静默失败卡住文档：

- **任务自动重试**：Celery 失败自动重试，复用统一的退避策略与状态码判断——只重试瞬时错误（嵌入 API 429/5xx、网络层故障），解析失败等永久错误直接失败（重放也必失败）。
...
- **KV 可插拔**：redis（生产）/ memory（测试），沿用 SparseIndex 接口的思路；全部读写 fail-open——缓存是优化不是依赖，Redis/Milvus 挂了问答照常走。

## 企业级加固

面向「可交付企业级」的一次补强，专治 demo 到生产的四个命门——**数据一致性、安全、责任、质量护栏**：

- **跨存储一致性**：Milvus 不参与 DB 事务，崩溃会留孤儿向量。入库顺序固定为「DB 先提交 → Milvus 后写 → 置 ready」，失败清残留；新增**对账任务**（admin 端点 + Celery）以 DB 为准清孤儿向量、报告缺向量——外部存储偏差可收敛，不留"幽灵数据"。
- **安全纵深**：JWT 加 jti 登出**黑名单**（令牌立即失效）；登录按「用户名+IP」令牌桶限流防爆破（超限 429+Retry-After）；生产环境占位 `SECRET_KEY` 直接拒绝启动。
- **责任可追溯**：`audit_logs` 记录 谁/何时/对什么做了什么，与业务**同事务**提交；admin 按操作人/动作分页查询——能删数据不留痕的系统进不了企业。
- **成本与质量护栏**：检索切片按 token 预算动态截断（顺带修掉重排关闭时 20 条全进 LLM 的 bug）+ `max_tokens` 输出上限；CI 每次提交离线跑黄金集评测，与 committed 基线对比**质量回退即 CI 红**——质量不是上线时测一次，是每次提交都在守。
- **多轮对话**：最近几轮历史喂给 LLM，追问（"那上限呢？"）才有上下文；多轮请求不进回答缓存（含上下文，缓存会答非所问）。
- **生产部署**：后端多阶段 + **非 root** 镜像、全服务 `restart` + 资源限制、backend 健康检查、uploads 命名卷（去掉源码 bind-mount）、CORS 白名单配置化。
- **真实企业脏文档**：演示文档升级为 8 份互相引用的企业制度集（员工手册/考勤休假/薪酬绩效/差旅报销/信息安全/采购合同/固定资产/离职交接），**刻意保留真实企业知识库里的脏数据**——页眉页脚残留、版本记录手写批注、新旧标准对照、OCR 乱码、错别字、半角全角标点混用、tab 对齐的"脏表格"、重复段落、TODO 待补、术语不统一（人力资源部/人事部/HR）等。每份 8-11 切片（触发 Markdown 标题感知切分），跨文档交叉引用天然产出多跳问题；黄金评测集升级到 52+3 题、基线 1.0 不漂移。

**生产环境故障排查（只在生产暴露的 bug）**：①PG 中文检索 `to_tsquery('simple', "'一线' OR '城市'")` 报 syntax error（SQLite 路径无恙）→ 改 `websearch_to_tsquery`；②seed 二次灌库撞 `chunks_fts` 主键（FTS 表不在 ORM 元数据里，drop_all 删不到）→ 补显式清稀疏索引。
...
- [x] 骨架与文档管线：上传 → 解析 → 切片 → 向量化 → 入库
- [x] 检索与问答：混合检索（BM25+向量+RRF）+ RAG 问答 + 引文标注
- [x] 真实 LLM 接入：OpenAI 兼容客户端（DeepSeek），提炼式回答 + 引文 + no_answer 哨兵句
- [x] 流式输出：SSE 逐字推送（meta/delta/done 事件）+ 前端打字机效果
- [x] Rerank 重排：召回(20)→重排→取前5→生成三层管线（RerankerProvider 接口抽象 + 真实 bge-reranker-v2-m3，词法实现保留作离线对照）
- [x] 多租户权限：RBAC 角色门卫 + 检索层元数据过滤 + 越权测试（含删除清向量）
- [x] 评测体系：黄金评测集（18 题）+ RAGAS 风格四指标 + 消融实验（纯向量/混合/重排/chunk 大小）
- [x] 评测集 v2：8 份企业脏文档 + 52 可答题/3 拒答题，新增拒答准确率指标（与四指标分开度量）
- [x] 工程规范：request_id 全链路追踪 + 令牌桶限流 + GitHub Actions CI
- [x] 边界打磨：外部依赖兜底 + 上传安全边界 + 输入校验
- [x] 版本管理：同文件名重传升版 + 旧切片双存储同步清理
- [x] 真实模型接入：bge-m3 嵌入 + bge-reranker 重排（SiliconFlow）+ 重跑真实评测
- [x] 生产化验收：Docker 全栈（Postgres + Milvus standalone + Celery）+ 数据重灌 + 全链路 E2E
- [x] 检索性能：关键词检索从全表 + 内存 BM25 换成数据库倒排索引（FTS5 / PG tsvector 双实现）
- [x] 容错重试：LLM/Embedding/Rerank 统一重试策略（429/5xx/网络错误重试，指数退避+抖动，SSE 刻意不重试）
- [x] 入库重试：文档处理失败自动重试（瞬时/永久错误分类）+ 失败原因落库 + 手动重试接口
- [x] 回答缓存：语义缓存热问题秒回（Milvus 问句向量 + KV 负载）+ 部门版本号失效 + 权限隔离
- [x] 企业级加固：跨存储一致性对账 + 认证安全（登出黑名单/登录防爆破/弱密钥拦截）+ 审计日志 + 上下文预算 + 非 root 部署
- [x] 迭代优化：多轮对话进生成 + Markdown 标题感知切片 + CI 评测护栏
- [x] 演示文档企业化：8 份互相引用的企业制度集 + 真实脏数据（页脚残留/OCR 乱码/新旧对照/错别字等）+ 黄金评测集 v2（52+3 题）跟随
