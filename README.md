...
- **多知识库**：文档按知识库组织，问答可在指定库内检索（可插拔即插即用）
- **文档生命周期**：解析 → 切片 → 嵌入 → 入库异步管线；重传升版、删除双存储同步清理
- **复杂文档增强**：扫描件 OCR（RapidOCR）、页码落块、类型化分块（按格式调块大小 + 句子边界对齐）
- **可信回答**：引文溯源 + 无答案识别（拒绝回答）+ **提示词防注入**（资料以 XML 定界符隔离，恶意指令视为数据不执行）
- **回答缓存**：语义缓存热问题秒回，降低 LLM 调用成本（缓存键带知识库维度）
- **查询改写**：rule / LLM 两种策略，改写只影响检索，提升同义表达召回
- **反馈闭环**：回答点赞/点踩 → 管理端汇总 → 坏例导出，反哺评测集
- **管理后台治理**：反馈/追踪单删/批量/清空、审计日志按时间清理（近期留痕保留）；删除仅 admin 且操作留痕，一键清空须输入 DELETE 二次确认
- **可观测与防护**：request_id 全链路追踪、**每次问答一条调用追踪（query_traces）**、**Prometheus 监控指标（问答/LLM/缓存/延迟）**、令牌桶限流、外部依赖容错重试
...
| OCR | RapidOCR（onnxruntime，扫描件识别） |
| 部署 | Docker Compose |
