...
用法：
  ...
  python scripts/ablation.py api         # LLM 裁判用真实 DeepSeek（花几分钱）
  python scripts/ablation.py api real    # 全真实：bge-m3 嵌入 + bge-reranker 重排 + DeepSeek 裁判

四组配置回答三个消融问题：
  1. 混合检索 vs 纯向量检索        → SEARCH_MODE hybrid|vector
  2. 有重排 vs 无重排              → RERANKER_BACKEND
...
REAL = len(sys.argv) > 2 and sys.argv[2] == "real"  # real: 真实 bge-m3 嵌入 + bge-reranker 重排
EMBED = "api" if REAL else "mock"
RERANK = "api" if REAL else "lexical"  # 混合+重排 组用的重排后端

# 消融配置：每组只改一个变量，其余保持生产默认
CONFIGS = [
    {
        "label": "混合检索+重排 (chunk500)",
        "env": {"SEARCH_MODE": "hybrid", "RERANKER_BACKEND": RERANK, "CHUNK_SIZE": "500"},
    },
    {
    ...
    },
    {
        ...
        "env": {"SEARCH_MODE": "hybrid", "RERANKER_BACKEND": RERANK, "CHUNK_SIZE": "300"},
    },
]
...
def run_one(index: int, cfg: dict) -> dict:
    ...
    print(f"\n>>> 运行配置 {index + 1}/{len(CONFIGS)}：{cfg['label']}（LLM={LLM} 嵌入={EMBED} 重排={cfg['env']['RERANKER_BACKEND']}）")
...
def main() -> None:
    ...
    if LLM == "mock":
        ...
        note = (
            "> 本次为 **全真实模式**：嵌入 bge-m3 + 重排 bge-reranker-v2-m3（SiliconFlow）+ LLM 裁判 DeepSeek，"
            "四个指标均为真实值，可直接用于调参。"
        )
    else:
        note = (
